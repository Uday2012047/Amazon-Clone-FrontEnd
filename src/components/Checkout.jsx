      ...
      <div className='min-w-[1000px] max-w-[1500px] m-auto pt-8'>
        <div className='grid grid-cols-8 bg-white'>
          ...
          <div className='col-span-6 bg-white'>
            <div className='text-2xl xl:text-3xl m-4'>Shopping Cart</div>
            {
              products.map(product => {
                return (
                  <div key={product.id}>
                    <div className='grid grid-cols-12 divide-y divide-gray-400 mr-5'>
                      ...
                      <div className='col-span-2'>
                        <div className='text-lg xl:text-xl mt-2 mr-4 font-semibold'>
                          {INR_CURRENCY.format(product.price)}</div>
                      </div>
                    </div>
                  </div>
                )
            ...
            <div className='text-lg xl:text-xl text-right mb-4 mr-5'>Subtotal ({itemsNumber}items):<span className='font-semibold'>{INR_CURRENCY.format(subtotal)}</span></div>
          </div>
          {/* checkout */}
          <div className="col-span-2 bg-white rounded h-[250px] p-7">
            ...
            <button class="bg-yellow-400 w-full p-3 text-xs xl:text-sm rounded hover:bg-yellow-500 mt-3">Proceed to Checkout</button>
          </div>
        </div>
      </div>
    </div>
